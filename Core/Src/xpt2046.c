...
/* ═══════════════════════════════════════════════════════════════════════════
   TOUCH TUNING — adjust these 7 defines until touch matches display exactly
   ═══════════════════════════════════════════════════════════════════════════

   HOW TO CALIBRATE (no tools needed, just the yellow debug string):

   STEP 1 — Raw limits
     Touch each of the 4 corners one at a time and read "raw(XXXX,YYYY)".
     • TOUCH_RAW_X_MIN  = smallest  rawX you see across all corners
     • TOUCH_RAW_X_MAX  = largest   rawX you see across all corners
     • TOUCH_RAW_Y_MIN  = smallest  rawY you see across all corners
     • TOUCH_RAW_Y_MAX  = largest   rawY you see across all corners
     This removes dead zones at the screen edges.

   STEP 2 — Axis swap  (TOUCH_SWAP_XY)
     Touch the LEFT edge and slowly drag RIGHT.
     • Crosshair moves left→right on screen : TOUCH_SWAP_XY = 0  (no swap)
     • Crosshair moves up→down   on screen  : TOUCH_SWAP_XY = 1  (swap)

   STEP 3 — Inversion  (TOUCH_INVERT_X / TOUCH_INVERT_Y)
     Touch the TOP-LEFT corner of the screen.
     • Crosshair appears top-left    : both correct, leave 0
     • Crosshair appears top-right   : TOUCH_INVERT_X = 1
     • Crosshair appears bottom-left : TOUCH_INVERT_Y = 1
     • Crosshair appears bottom-right: both = 1

   ─── Default starting point for LANDSCAPE (MADCTL = MV|MX|MY|BGR) ───────
       SWAP=1, INVERT_X=0, INVERT_Y=1
   ═══════════════════════════════════════════════════════════════════════════ */

#define TOUCH_RAW_X_MIN   300u   /* down raw X at left/top edge   — tune me    */
#define TOUCH_RAW_X_MAX   1800u   /* raw X at right/bot edge  — tune me    */
#define TOUCH_RAW_Y_MIN    300u   /* -> raw Y at left/top edge   — tune me    */
#define TOUCH_RAW_Y_MAX   1800u   /* raw Y at right/bot edge  — tune me    */

#define TOUCH_SWAP_XY      1u     /* 1 = swap raw X↔Y axes                 */
...
bool XPT2046_GetTouchPixel(XPT2046_Point_t *pt)
{
    ...
    if (_calib.valid) {
        ...
        /* ── Uncalibrated fallback — controlled by tuning defines above ─── */

        /* Clamp raw to known panel range */
        if (rawX < TOUCH_RAW_X_MIN) rawX = TOUCH_RAW_X_MIN;
        if (rawX > TOUCH_RAW_X_MAX) rawX = TOUCH_RAW_X_MAX;
        if (rawY < TOUCH_RAW_Y_MIN) rawY = TOUCH_RAW_Y_MIN;
        if (rawY > TOUCH_RAW_Y_MAX) rawY = TOUCH_RAW_Y_MAX;
        ...
        /* Linear map to pixel coordinates */
        uint16_t px = (uint16_t)(((uint32_t)(axX - axX_min) * ILI9488_WIDTH)
                                 / (axX_max - axX_min));
        uint16_t py = (uint16_t)(((uint32_t)(axY - axY_min) * ILI9488_HEIGHT)
                                 / (axY_max - axY_min));

        /* Axis inversion */
#if TOUCH_INVERT_X
        px = (ILI9488_WIDTH  - 1u) - px;
#endif
#if TOUCH_INVERT_Y
        py = (ILI9488_HEIGHT - 1u) - py;
#endif
